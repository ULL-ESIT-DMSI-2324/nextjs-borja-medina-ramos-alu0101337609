...
export default function RandomImageGenerator() {
  ...
  async function generateRandomImage() {
    try {
      ...
      if (!response.ok) {
        throw new Error("Error al generar la imagen. Por favor, inténtelo de nuevo.");
      }
      ...
      setError("Error al conectarse al servidor. Por favor, inténtelo de nuevo más tarde.");
    }
  }
  ...
  return (
    <div className={styles.container}>
      ...
      <section className={styles.main} style={{ backgroundColor: '#f9f9f9', padding: '20px', borderRadius: '10px' }}>
        <h1 className={styles.title} style={{ color: '#ff5a5f' }}>Generador de Imágenes</h1>
  
        <form className={styles.form} onSubmit={(e) => { e.preventDefault(); generateRandomImage(); }}>
          <label htmlFor="imageName" className={styles.label} style={{ color: '#333' }}>Nombre de la Imagen:</label>
          <input
            type="text"
            id="imageName"
            value={imageName}
